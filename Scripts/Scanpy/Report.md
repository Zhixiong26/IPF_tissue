...
现有 `Scripts/01_run_e_scanpy.py` 和 `Scripts/run_e_scanpy.sbatch` 属于 notebook 验证前草稿，不作为正式分析提交。notebook 确认后再记录实际过滤数量、归一化/表示、邻接图、聚类、嵌入、QC 图、命令、环境及解释限制，并据此重写脚本。
