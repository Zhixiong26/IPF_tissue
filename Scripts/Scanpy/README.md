...
- `Scripts/`: versioned Python and Slurm entry points created after notebook validation.
...
`Scripts/01_run_e_scanpy.py` and `Scripts/run_e_scanpy.sbatch` are provisional drafts created before notebook validation. Do not submit them as the final analysis. Update them only after the notebook parameters and outputs are confirmed.
