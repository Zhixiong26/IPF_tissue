...
- 正式提交前检查已通过：输出根 `/mnt/data04/luozhixiong/IPF_tissue/Results/QC/full_counts_20260824` 已创建且为空，`/mnt/data04` 约有 4.5 TiB 可用空间；当前纳入的 CYL、ZCP 各有 156 BAM、2 R1、2 R2；Samtools 1.22 和两个 sbatch 脚本验证正常。 / Submission preflight passed: the output root `/mnt/data04/luozhixiong/IPF_tissue/Results/QC/full_counts_20260824` has been created and is empty, `/mnt/data04` has about 4.5 TiB free, and the included CYL and ZCP samples each have 156 BAMs, 2 R1s, and 2 R2s; Samtools 1.22 and both sbatch scripts validated successfully.
- Slurm `sbatch --test-only` 接受 `fat` partition 上的两个 array 资源请求，并确认目标节点为 `fat01`；最近测试显示的 307473/307474 不是已创建作业。正式提交仍等待使用者确认。 / Slurm `sbatch --test-only` accepted both array resource requests on the `fat` partition and selected `fat01`; the latest displayed test identifiers 307473/307474 are not created jobs. Formal submission still awaits user confirmation.
