...
5. **MethSCAn.** For ALLC intake, VMR discovery, matrix generation, Scanpy embedding/clustering, smoke tests, submission, or interpretation, read [methscan.md](references/methscan.md). Start from the user-supplied ALLCools ALLCs/archive and use MethSCAn's native ALLC input; keep QC selection optional until its table and Boolean column are verified.
