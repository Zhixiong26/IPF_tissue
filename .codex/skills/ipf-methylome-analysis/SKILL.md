...
5. **Methylome Scanpy / ALLCools.** For MethylVI routes, read [methylvi.md](references/methylvi.md). Use the staged 5-kb ALLCools route and the MethSCAn-VMR route only after the paired MethSCAn run is complete. Inspect H5AD metadata, dimensions, QC, neighbours, embeddings, and figure manifests before interpreting clusters. Do not confuse this methylome workflow with CYL/ZCP transcriptome-E Scanpy.
6. **MethylVI.** Use the one-click `Scripts/Methylvi/submit_methylvi_8models.sh` DAG for production: four feature builders, eight dependent model jobs, and a final summary gate. Use integer methylated-count and coverage layers built from indexed ALLCs, not the ALLCools hypo-score matrix. Validate integral counts and `mc <= cov`, retain source manifests, and treat checkpoint reuse as valid only when its cell/feature/config signatures match. The maintained experiment uses 6,264 MethSCAn post-filter cells, ALLCools 5-kb × {10k,30k}, and VMR {0.01,0.02,0.05} × {10k,30k}. Do not silently substitute historical 30wcov-derived ALLCs for the current selected-ALLC manifest.
...
- For VMR methylation UMAP figures, use an explicit palette with at least as many colors as cell-type labels; matplotlib's default 10-color cycle is insufficient for the project's 14 labels. Treat supervised UMAP `target_weight` values above about 0.3 as sensitivity analyses because they can collapse or distort the unsupervised latent geometry; use ordinary UMAP for the primary methylation structure and label-guided UMAP only as an auxiliary view.
